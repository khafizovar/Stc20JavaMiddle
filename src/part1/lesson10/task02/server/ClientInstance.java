...
public class ClientInstance extends Thread {
    ...
    public ClientInstance(Socket s, DataInputStream dis, DataOutputStream dos, Boolean udpBroadcast) {
        this.s = s;
        this.dis = dis;
        this.dos = dos;
    ...
    private void broadcast(String message) throws IOException {
        String sMess = "[" + this.clientName + "] - [" + timeFormat.format(new Date()) + "]: " + message;
        if(!udpBroadcast) {
            for (ClientInstance mc : Server.getConnectedClients()) {
                if (mc.isConnected() && mc.isRegistered())
                    mc.dos.writeUTF(sMess);
            }
        } else
            Server.broadCastMessage(sMess);
    }
